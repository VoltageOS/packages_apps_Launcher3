...
public class StashedHandleViewController implements TaskbarControllers.LoggableTaskbarController,
        ...
        if (activity.isPhoneGestureNavMode() || activity.isTinyTaskbar()
            ...
            if (handleWidthMode == 0) {
                mStashedHandleWidth = resources
                        .getDimensionPixelSize(R.dimen.taskbar_stashed_small_screen_short);
            } else if (handleWidthMode == 2) {
                mStashedHandleWidth = resources
                        .getDimensionPixelSize(R.dimen.taskbar_stashed_small_screen_long);
            } else {
                mStashedHandleWidth = resources
                        .getDimensionPixelSize(R.dimen.taskbar_stashed_small_screen);
            }
        } else {
    ...
    private boolean shouldAutoHideHandle() {
        ...
        return activity != null
                && !activity.isThreeButtonNav()
                && SettingsCache.INSTANCE.get(activity).getValue(NAVIGATION_BAR_HINT_URI)
                && Settings.System.getIntForUser(activity.getContentResolver(),
